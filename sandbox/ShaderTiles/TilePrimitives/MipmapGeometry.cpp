...
namespace osgOcean
{
    ...
    MipmapGeometry::MipmapGeometry( unsigned int numLevels, float worldSize ):
    ...
    osg::BoundingBox MipmapGeometry::computeBound( void ) const 
    {
        osg::BoundingBox bb;
    ...
    bool MipmapGeometry::updatePrimitives( unsigned int level, unsigned int levelRight, unsigned int levelBelow )
    {
    ...
    bool MipmapGeometry::checkPrimitives( unsigned int level, unsigned int levelRight, unsigned int levelBelow )
    {
#ifndef NDEBUG 
        if( abs( (int)level-(int)levelRight) > 1 || 
            abs( (int)level-(int)levelBelow) > 1 )
        {   
            osg::notify(osg::WARN) << "osgOcean::MipmapGeometry() - Mipmap level difference is greater than 1" << std::endl;
        }
#endif 
    ...
    void MipmapGeometry::addMainBody( void )
    {
        _mainBody.clear();
        ...
        for(unsigned r = 0; r < rowLimit; r+=inc )
        {
            ...
            int i = -1;
            for(unsigned c = 0; c < colLimit; c+=inc)
            {
                (*primitive)[++i] = getIndex( c, r     );
                (*primitive)[++i] = getIndex( c, r+inc );
            }
    ...
    void MipmapGeometry::addZeroTile( void )
    {
        _mainBody.clear();
        _belowBorder.clear();
        _rightBorder.clear();
    ...
    void MipmapGeometry::addZeroCornerPiece(void)
    {
        _belowBorder.clear();
        _rightBorder.clear();
        _cornerPiece.clear();
    ...
    void MipmapGeometry::addBottomBorder(void)
    {
        _belowBorder.clear();
    ...
    void MipmapGeometry::addRightBorder( void )
    {
        _rightBorder.clear();
    ...
    void MipmapGeometry::addCornerPiece(void)
    {
        _cornerPiece.clear();
        ...
        else if( _levelBelow >= _level && _levelRight <= _level )
        {
            ...
            for(int r = _maxResolution; r >= _maxResolution-inc; r-=incRight )
                primitive->push_back( getIndex( _maxResolution, r ) );

            for(int c = _maxResolution; c >= _maxResolution-incBelow; c-=inc )
    ...
    void MipmapGeometry::assignPrimitives( void )
    {
