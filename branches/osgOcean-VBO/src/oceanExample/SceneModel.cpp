   ...
   bool  useVBO )
{
    ...
    build(windDirection, windSpeed, depth, reflectionDamping, scale, isChoppy, choppyFactor, crestFoamHeight, _useVBO);
}

void SceneModel::build( 
    const osg::Vec2f& windDirection,
    float windSpeed,
    float depth,
    float reflectionDamping,
...
{
    {
        ...
        // Set up surface
        {
            ...
            else
            {
                ...
                oceanSurface->setEnvironmentMap( _cubemap.get() );
                oceanSurface->setFoamBottomHeight( 2.2f );
                oceanSurface->setFoamTopHeight( 3.0f );
                oceanSurface->enableCrestFoam( true );
                oceanSurface->setLightColor( _lightColors[_sceneType] );
                // Make the ocean surface track with the main camera position, giving the illusion
                // of an endless ocean surface.
                oceanSurface->enableEndlessOcean(true);
...
void SceneModel::changeScene( SCENE_TYPE type )
{
    _sceneType = type;
    ...
    if( _useVBO )
    {
        osgOcean::FFTOceanSurfaceVBO* oceanSurfaceVBO = static_cast< osgOcean::FFTOceanSurfaceVBO* >( _oceanSurface.get() );
        oceanSurfaceVBO->setEnvironmentMap( _cubemap.get() );
        oceanSurfaceVBO->setLightColor( _lightColors[type] );
    }
    else
    {
        osgOcean::FFTOceanSurface* oceanSurface = static_cast< osgOcean::FFTOceanSurface* >( _oceanSurface.get() );
        oceanSurface->setEnvironmentMap( _cubemap.get() );
        oceanSurface->setLightColor( _lightColors[type] );
    }
...
osg::Geode* SceneModel::sunDebug( const osg::Vec3f& position )
{
