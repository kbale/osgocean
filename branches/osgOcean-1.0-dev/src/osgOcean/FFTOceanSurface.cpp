...
void FFTOceanSurface::computeSea( unsigned int totalFrames )
{
	...
	for( unsigned int frame = 0; frame < totalFrames; ++frame )
	{
		...
		// Level 0
		if(_isChoppy)
			_mipmapData[frame][0] = OceanTile( heights, displacements, _tileSize, _pointSpacing );
		else
			_mipmapData[frame][0] = OceanTile( heights, _tileSize, _pointSpacing );
...
osg::Program* FFTOceanSurface::createShader(void)
{
	...
	vShader->setName("ocean_surface_vertex_shader");

	osg::ref_ptr<osg::Shader> fShader = new osg::Shader( osg::Shader::FRAGMENT );
	fShader->setName("ocean_surface_fragment_shader");

	std::string vertPath = osgDB::findDataFile("water.vert");

	if(vertPath.empty())
		return NULL;

	std::string fragPath = osgDB::findDataFile("water.frag");

	if(fragPath.empty())
		return NULL;

	vShader->loadShaderSourceFromFile(vertPath);
	fShader->loadShaderSourceFromFile(fragPath);
